...
You can check our example in the [Devpod's AWS Provider](https://dev.khulnasoft.com-provider-aws/blob/main/pkg/options/options.go#L44)
...
- `hidden`: If true, DevPod will not show this option in the Desktop application or through `devspace provider options`. Can be used to calculate variables internally or save tokens or other things internally.
