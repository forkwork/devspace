...
If you have selected an IDE to automatically open, DevPod will automatically try to open the IDE after a successful workspace creation or `devspace up` command.
...
Upon workspace creation, DevPod will automatically modify the `~/.ssh/config` to include an entry for `WORKSPACE_NAME.devspace`, which allows you to use the following command to connect to your workspace:
```
